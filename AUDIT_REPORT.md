...
| `lib/telegram.ts` | Общая доставка сообщений владельцу (релей + прямой вызов) |
| `app/payment/checkout-fields.tsx` | Поля чекаута с выбором типа покупателя |
| `app/payment/invoice/page.tsx` | Страница счёта для ИП и организаций |
| `scripts/confirm-bank-payment.mjs` | Административное подтверждение B2B-оплаты |
| `tests/bank-payment.test.mjs` | 6 тестов административного подтверждения |
...
| Tests | `node --test tests/*.test.mjs` | ✅ **80 / 80** |
...
## 14. Два платёжных потока: физлица и организации

### Ответ поддержки Robokassa

> «Наш сервис может принимать оплаты только от физических лиц.»

Это снимает вопрос о передаче ИНН покупателя в `Receipt`: B2B через Robokassa не
проводится в принципе. Ранее найденное отсутствие полей покупателя в API
(`Receipt` содержит только `sno` и `items`, тегов 1227/1228 нет нигде) оказалось
следствием именно этого ограничения, а не пробелом в документации.

### Архитектура

```
                    ┌── buyerType=individual ──► Robokassa ──► ResultURL ──► paid ──┐
checkout ──validate─┤                              (карта/СБП, чек НПД авто)        ├──► access_grant ──► /brief
                    └── buyerType=business ────► /payment/invoice ──► банк ──► CLI ─┘
                                                  (счёт, чек НПД вручную)
```

| | Физлицо | ИП / организация |
|---|---|---|
| Оплата | карта, СБП через Robokassa | банковский перевод по счёту |
| Статус при создании | `pending` | `awaiting_bank_payment` |
| Что подтверждает оплату | ResultURL + подпись Password#2 | `scripts/confirm-bank-payment.mjs` |
| Robokassa | подпись, `Receipt`, `IsTest` | **не вызывается вообще** |
| Чек НПД | автоматически, Робочеки СМЗ | вручную в «Мой налог», с ИНН покупателя |
| Доступ | `access_grant`, 7 дней от `paid_at` | тот же механизм, те же 7 дней |

Оба пути сходятся в одной точке: `status='paid'` + `access_grant`. Дальше
`/brief` и `/api/intake` не различают, как именно платёж поступил.

### Почему business-заказ не может быть оплачен через Robokassa

Три независимых барьера:

1. `/api/payment/start` для business возвращает 303 на `/payment/invoice` —
   подпись не считается, `Receipt` не строится, реквизиты покупателя в Robokassa
   не уходят.
2. `confirmPayment()` отклоняет заказ с `buyer_type='business'` до сверки суммы,
   callback получает **409** и `OK` не возвращается.
3. Условный UPDATE дополнительно требует `status='pending' AND buyer_type='individual'`,
   то есть даже при обходе проверки выше строка не изменится.

Симметрично: административный CLI требует `status='awaiting_bank_payment'`, чего
у карточного заказа не бывает. Рельсы не пересекаются ни в одну сторону.

### Подтверждение B2B-оплаты

Публичного endpoint нет намеренно. Подтверждение выполняется инструментом,
которому нужны учётные данные БД:

```bash
DATABASE_URL='postgresql://…?sslmode=verify-full'   node scripts/confirm-bank-payment.mjs <НОМЕР_ЗАКАЗА>
```

Инструмент в одной транзакции переводит `awaiting_bank_payment → paid`, ставит
`paid_at` и создаёт ровно один `access_grant`. Повторный запуск ничего не меняет.
Отказывается работать с заказом физлица и с неизвестным номером. Покрыт
`tests/bank-payment.test.mjs` — запускается как настоящий дочерний процесс.

### Уведомление о новом B2B-заказе

Карточный платёж объявляет о себе callback-ом от Robokassa; у банковского
перевода такого сигнала нет. Поэтому сразу после создания business-заказа
владельцу уходит сообщение в Telegram — тем же механизмом, что и анкеты, без
новых внешних сервисов. Логика доставки вынесена из `/api/intake` в `lib/telegram.ts`
и используется обоими путями.

В сообщении: номер заказа, наименование плательщика, ИНН, email, тариф, сумма,
статус `awaiting_bank_payment` и готовая команда подтверждения. Секретов нет —
ни сессионного хеша, ни строки подключения, ни паролей Robokassa; это закреплено
отдельным тестом.

Отказ Telegram **не отменяет заказ**: строка уже зафиксирована, покупатель
получает страницу счёта, а сбой попадает в лог как код причины. Проверено тремя
сценариями — отказ API, обрыв сети и полностью ненастроенный Telegram.

Ручной порядок для владельца:

1. Получить уведомление в Telegram и выставить счёт на указанный email.
2. Увидеть поступление на расчётный счёт с номером заказа в назначении платежа.
3. Выполнить команду выше.
4. Сформировать чек НПД в «Мой налог» на покупателя, указав его ИНН.
5. Покупатель открывает `/payment/invoice` или `/brief` в том же браузере.

### Что изменено в текстах

- Оферта, п. 5.2–5.4: разведены два способа оплаты, для юрлиц указано, что чек
  НПД формируется после поступления оплаты с указанием ИНН Заказчика.
- Лендинг и подвал: обещание автоматического чека теперь относится только к
  физлицам.
- `ROBOKASSA_B2B_RECEIPT_CONFIRMED` — имя историческое и вводит в заблуждение:
  к B2B-чекам переменная отношения больше не имеет, она лишь второе явное
  подтверждение для открытия боевых карточных платежей. Переименование
  потребовало бы смены production ENV, поэтому имя сохранено, а смысл описан в
  `.env.example`.

### Миграция

Новая **не нужна**. `awaiting_bank_payment` — значение текстовой колонки
`status`, ограничения на неё нет; `drizzle-kit generate` подтверждает отсутствие
изменений схемы. Достаточно уже созданной `0001_previous_speed.sql`.

### Остаётся открытым

- Реквизиты для перевода задаются через `BANK_TRANSFER_DETAILS`; пока переменная
  не задана, страница счёта сообщает, что счёт будет выслан на email.
- Выставление чека НПД юрлицу — ручная операция. Автоматизации нет и, судя по
  ответу поддержки, через Robokassa не будет.
- Сверка поступлений с банком не автоматизирована: подтверждение инициирует
  человек. Уведомление сообщает о заказе, но не об оплате.
