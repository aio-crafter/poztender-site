...
export async function sendOwnerMessage(env, text, logPrefix) {
  ...
  if (!relayResponse) {
    // The one state that used to leave no trace at all: the relay was skipped
    // because TELEGRAM_RELAY_URL is not an https URL or TELEGRAM_RELAY_SECRET
    // is shorter than 32 characters. On a host that cannot reach
    // api.telegram.org this is the difference between delivery and silence,
    // so it must be visible in the log rather than inferred from its absence.
    console.error(
      `${logPrefix} telegram relay is not configured (needs https TELEGRAM_RELAY_URL and a 32+ character TELEGRAM_RELAY_SECRET); trying the direct Telegram API`,
    );
  }
  if (relayResponse) {
