...
- `ROBOKASSA_B2B_RECEIPT_CONFIRMED=true` — второе явное подтверждение для открытия боевых карточных платежей. Имя историческое: Robokassa принимает оплату только от физлиц, поэтому к B2B-чекам переменная отношения не имеет. Для тестовой оплаты оставьте `false`.

Пока последняя переменная не установлена, кнопка оплаты безопасно переводит на страницу «подключается». ИП и организации оплачивают банковским переводом — Robokassa для них не используется, см. `DEPLOY_TIMEWEB.md`.
